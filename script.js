...
displayDefaultLocation = () => {
  // MEDAN 3.585274, 98.681337  
  const lat = document.getElementById('latitude_input').value == "0" ? 3.585274 : parseFloat(document.getElementById('latitude_input').value);
  const lng = document.getElementById('longitude_input').value == "0" ? 98.681337 : parseFloat(document.getElementById('longitude_input').value);
...
getGeolocation = (lat, lng) => {
  ...
  fetch(`https://maps.googleapis.com/maps/api/geocode/json?latlng=${latlng}&key=${__KEY}`)
    ...
    .then(res => res.json()).then(data => inputClicked(data.results[0]));
...
inputClicked = (result) => {
  ...
  document.getElementById("latitude_input").value = result.geometry.location.lat;
  document.getElementById("longitude_input").value = result.geometry.location.lng;
  /* Tambahan biar setiap ada perubahan, lat dan lng berubah juga
  */
  // result.geometry.map(component => {
  //   const location = component.location
  //   if (location.includes('lat')) {
  //     console.log(location.lat);
  //     $('latitude_input').value = component.value;
  //   }    
  // }); 
...
document.getElementsByTagName('body')[0].onload = displayDefaultLocation();